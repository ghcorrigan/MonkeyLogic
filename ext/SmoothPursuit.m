...
%     end
    ...
    methods
        ...
        function set.Target(obj,target)
            ...
            obj.Target = target;
            obj.TargetID = obj.Tracker.TaskObject.ID(target);
        end
        ...
        function init(obj,p)
            obj.Adapter.init(p);
            obj.Success = false;
            obj.OriginalPosition = obj.Tracker.TaskObject.Position(obj.Target,:); %#ok<*MCSUP>
        ...
        function fini(obj,p)
            ...
            obj.Tracker.TaskObject.Position(obj.Target,:) = obj.OriginalPosition;
        end
        function continue_ = analyze(obj,p)
            ...
            if p.scene_time() < obj.Duration
                ...
                in = all(sum((data-repmat(obj.ScrTarget,ndata,1)).^2,2) < obj.ThresholdInPixels);
                if ~in, continue_ = false; return, end
            else
                obj.Success = true;
                continue_ = false;
                return
